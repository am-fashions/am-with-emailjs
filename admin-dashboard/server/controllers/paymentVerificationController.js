...
export const submitPaymentVerification = async (req, res) => {
  try {
    ...
    // Call stored procedure to submit verification
    ...
    const verification_id = result[0][0].verification_id;
...
export const getVerificationById = async (req, res) => {
  try {
    const verification = await db.getOne(
      `SELECT * FROM vw_all_verifications WHERE verification_id = ?`,
      [req.params.id]
    );
    ...
    const logs = await db.getMany(
      `SELECT * FROM payment_verification_logs WHERE verification_id = ? ORDER BY created_at DESC`,
      [req.params.id]
    );
...
export const verifyPayment = async (req, res) => {
  try {
    ...
    // Call stored procedure to verify payment
    await db.query(
      `CALL sp_verify_payment(?, ?, ?)`,
      [verification_id, admin_id, admin_notes || 'Payment verified']
    );
...
export const rejectPayment = async (req, res) => {
  try {
    ...
    // Call stored procedure to reject payment
    await db.query(
      `CALL sp_reject_payment(?, ?, ?)`,
      [verification_id, admin_id, rejection_reason]
    );
...
export const getVerificationByOrderId = async (req, res) => {
  try {
    const verification = await db.getOne(
      `SELECT * FROM vw_all_verifications WHERE order_id = ? ORDER BY submitted_at DESC LIMIT 1`,
      [req.params.orderId]
    );
